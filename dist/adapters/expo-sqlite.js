...
exports.createScopedExec = createScopedExec;
exports.createExpoSqliteAdapter = createExpoSqliteAdapter;
const READ_REGEX = /^\s*(?:SELECT|PRAGMA|EXPLAIN|WITH)\b/i;
const NESTED_TRANSACTION_ERROR = 'createExpoSqliteAdapter: nested transactions are not supported (savepoints are out of scope). ' +
...
function createScopedExec(execute) {
    ...
    scopedExec.runInTransaction = async (_fn) => {
...
function createExpoSqliteAdapter({ databaseName, database } = {}) {
    if (databaseName === undefined && database === undefined) {
        return _throwInputError('createExpoSqliteAdapter: exactly one of "databaseName" or "database" is required, but neither was given.');
    }
    if (databaseName !== undefined && database !== undefined) {
        return _throwInputError('createExpoSqliteAdapter: exactly one of "databaseName" or "database" is required, not both.');
    }
    let db = database;
    ...
    const execute = async function execute(sql, params = [], options = {}) {
    ...
    const run = async function run(sql, params = [], options = {}) {
